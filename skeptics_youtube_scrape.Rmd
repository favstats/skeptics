...
pacman::p_load(tidyverse, tuber, magrittr, stringi, qdapRegex, purrr, lubridate, ggthemes, httr, plyr, svMisc, janitor)
...
repzion <- read_file("text/repzion.txt")
warski <- read_file("text/warski.txt")
blairewhite <- read_file("text/blairewhite.txt")
cultofdusty <- read_file("text/cultofdusty.txt")
bps <- read_file("text/bps.txt")
undoomed <- read_file("text/undoomed.txt")
```
...
repzion_ids <- extract_video_ids(stri_enc_toutf8(repzion))
warski_ids <- extract_video_ids(stri_enc_toutf8(warski))
blairewhite_ids <- extract_video_ids(stri_enc_toutf8(blairewhite))
cultofdusty_ids <- extract_video_ids(cultofdusty)
bps_ids <- extract_video_ids(bps)
undoomed_ids <- extract_video_ids(undoomed)
...
head(repzion_ids)
head(warski_ids)
head(blairewhite_ids)
head(cultofdusty_ids)
head(bps_ids)
```
# GetAllComments
