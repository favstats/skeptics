...
extract_comments <- function(ids, n, youtube) {
  comments <- list()
  for (jj in n:length(ids)) {
    ...
    if (is.data.frame(outcome)) {
      ...
      Sys.sleep(300)                
      if (file.exists(auth_token)) {  # IS ALWAYS TRUE NEEDS FIXING
...
amazingatheist_comments <- supresswarni(bind_rows(amazingatheist_comments))
```
